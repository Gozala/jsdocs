...
exports.publish = function publish(symbolSet, files) {
    ...
    classes.map(function(data) {
        ...
        docs[escapePath(path + '@' + element.name + '.html')] = classTemplate.render(element);
    });
    ...
    files.forEach(function(file) {
        try {
            ...
            var code = Plugins.syntaxHighlight(file);
            Log.debug('rendering source template for : ' + file);
            docs[escapePath(file + '.html')] = codeTemplate.render({
                page: {
                    title: title,
                    style: css,
                    ieFix: ieFix,
                ...
                lines: code.split('<br/>').map(function(value, index) {
                    return ++index;
                }),
                // subtemplate
                list: list
            ...
            Log.error('Could not highlight source for : ' + file + ' : ' + e);
        }
    });
