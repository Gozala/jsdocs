...
exports.doc = function doc(options) {
    ...
    var templatesDirectory = catalog["jsdocs"].directory.join("templates");
    var templatePath = options.template || "default";
    var template = templatesDirectory.join(templatePath); // trying to find in templates first
    if (!template.exists()) template = fs.path(templatePath); // trying absolute path if not in templates
    if (!template.exists()) throw new Error("Specified template doesn't exists: " + template.absolute().toString())
    options.template = template;
    console.info("Template used: '" + template.absolute().toString() + "'.");
    ...
    require(options.template.join("publish").absolute().toString()).publish(symbols, options);
};
/**
